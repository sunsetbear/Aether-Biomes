...
public class AetherBiomesGrassBlock extends GrassBlock {
    public AetherBiomesGrassBlock(Properties properties) {
        super(properties);
    ...
    public static void blockColorLoad(RegisterColorHandlersEvent.Block event) {
        event.getBlockColors().register((bs, world, pos, index) -> world != null && pos != null ? BiomeColors.getAverageGrassColor(world, pos) : GrassColor.get(0.5D, 1.0D), AetherBiomesBlocks.AETHER_BIOMES_GRASS_BLOCK.get());
    }


    @OnlyIn(Dist.CLIENT)
    public static void itemColorLoad(RegisterColorHandlersEvent.Item event) {
        event.getItemColors().register((stack, index) -> GrassColor.get(0.5D, 1.0D), AetherBiomesBlocks.AETHER_BIOMES_GRASS_BLOCK.get());
    }
}
